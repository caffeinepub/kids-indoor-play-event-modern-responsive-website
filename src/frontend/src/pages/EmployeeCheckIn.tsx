...
import { AlertCircle, CheckCircle, Home, Search } from "lucide-react";
...
export default function EmployeeCheckIn() {
  ...
  const { data: searchResults = [], isLoading: searchLoading } =
    useSearchWaiversForCheckIn(searchTerm);
...
function CheckInCard({ waiver, onCheckIn, isCheckingIn }: CheckInCardProps) {
  const _childrenNames = waiver.children.map((c) => c.name).join(", ");
            ...
            <div className="bg-gray-50 p-2 rounded border border-gray-200">
              ...
              <div className="space-y-1">
                {waiver.children.map((child) => (
                  <p
                    key={
                      child.name || child.birthday || Math.random().toString()
                    }
