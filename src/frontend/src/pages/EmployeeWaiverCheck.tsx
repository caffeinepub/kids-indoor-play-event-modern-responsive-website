...
export default function EmployeeWaiverCheck() {
  const navigate = useNavigate();
  const [searchTerm, setSearchTerm] = useState("");
  const [expandedWaiverId, setExpandedWaiverId] = useState<string | null>(null);

  // Fetch recent waivers (auto-refreshes every 5 seconds) - PUBLIC endpoint
  const {
    data: recentWaivers = [],
  ...
  // Search waivers - PUBLIC endpoint
          ...
          <CardContent>
            {recentError && (
              <Alert variant="destructive" className="mb-4">
                ...
                <AlertDescription>
                  Failed to load waivers. Please try again later.
                </AlertDescription>
              </Alert>
            )}

            {(recentLoading && !recentWaivers.length) ||
        ...
        <div className="flex-1 min-w-0 space-y-1">
          <div className="flex flex-col sm:flex-row sm:items-center gap-1 sm:gap-3">
            ...
            <span className="text-xs text-gray-500 bg-blue-50 px-2 py-0.5 rounded-full w-fit">
              {waiver.children.length}{" "}
              {waiver.children.length === 1 ? "child" : "children"}
            </span>
          </div>
          <div className="text-xs text-gray-600 space-y-0.5">
            <p className="truncate">
              {waiver.parentEmail} • {waiver.parentPhone}
            </p>
          ...
          <div>
            ...
            <div className="space-y-1.5">
              {waiver.children.map((child) => (
                <div
                  key={child.name || child.birthday || Math.random().toString()}
