...
interface BlogTabProps {
  ...
  saveBlogArticleMutation: {
    isPending: boolean;
    mutate: (data: Partial<BlogArticle>) => void;
  };
  ...
  return (
    <div className="bg-white rounded-2xl shadow-xl p-8 border-2 border-emerald-200">
        ...
        <div className="flex items-start">
          ...
          <div>
            <h4 className="text-lg font-semibold text-emerald-900 mb-3">
              ✍️ Create Engaging Content for Parents
            </h4>
            <div className="space-y-2 text-sm text-emerald-800">
              <p>
                <strong>📝 Share valuable insights:</strong> Create articles
                about parenting tips, child development, and family activities
              </p>
              <p>
                <strong>🎯 Build community:</strong> Connect with parents
                through helpful resources and local event information
              </p>
              <p>
                <strong>📈 Boost SEO:</strong> Regular content helps families
                find your indoor playground online
              </p>
              <p>
                <strong>🎉 Showcase expertise:</strong> Position yourself as a
                trusted resource for Oklahoma City families
              </p>
            </div>
            <div className="mt-3 p-3 bg-blue-50 border border-blue-200 rounded-lg">
              <p className="text-xs text-blue-800">
                <strong>💡 Content Ideas:</strong> Birthday party planning tips,
                indoor play benefits, safety guidelines, local family events,
                seasonal activities, and developmental milestones.
              </p>
            </div>
          </div>
        </div>
      ...
      <div className="mb-8 p-6 bg-gradient-to-br from-emerald-50 to-teal-50 rounded-xl border-2 border-emerald-200">
        ...
        >
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              ...
              <p className="text-xs text-gray-500 mt-1">
                {blogForm.title?.length || 0}/100 characters
              </p>
            </div>
            <div>
              <label
              ...
              />
              <p className="text-xs text-gray-500 mt-1">
                {blogForm.seoTitle?.length || 0}/60 characters
              </p>
            </div>
          </div>
          ...
          <div>
            ...
            />
            <p className="text-xs text-gray-500 mt-1">
              {blogForm.excerpt?.length || 0}/200 characters
            </p>
          </div>
          ...
          <div>
            ...
            <textarea
              ...
              onChange={(e) =>
                setBlogForm({ ...blogForm, seoDescription: e.target.value })
              }
              rows={2}
              className="w-full px-4 py-3 border-2 border-emerald-200 rounded-xl focus:ring-2 focus:ring-emerald-500 focus:border-transparent transition-all"
            ...
            />
            <p className="text-xs text-gray-500 mt-1">
              {blogForm.seoDescription?.length || 0}/160 characters
            </p>
          </div>
              ...
              {blogForm.featuredImage ? (
                <div className="space-y-2">
                  <p className="text-sm text-emerald-600 flex items-center justify-center">
                    <CheckCircle className="w-4 h-4 mr-2" />✅ Featured image
                    uploaded
                  </p>
                  ...
                  <button
                    type="button"
                    onClick={() =>
                      setBlogForm({ ...blogForm, featuredImage: undefined })
                    }
      ...
      <div>
        <div className="flex items-center justify-between mb-6">
          <h4 className="text-lg font-semibold text-gray-900 flex items-center">
            <BookOpen className="w-5 h-5 mr-2" />📚 Your Articles (
            {blogArticles.length})
          </h4>
        ...
        {blogArticles.length > 0 ? (
          <div className="space-y-4">
            {blogArticles.map((article) => {
              ...
              return (
                <div
                  key={article.id}
                  className="border-2 border-emerald-200 rounded-xl p-4 bg-gradient-to-r from-emerald-50 to-teal-50 hover:shadow-lg transition-all"
                >
                  <div className="flex items-start justify-between">
                    <div className="flex-1">
                      <div className="flex items-center gap-2 mb-2">
                        ...
                        <span
                          className={`px-2 py-1 rounded-full text-xs font-medium ${article.status === "published" ? "bg-green-100 text-green-800" : "bg-yellow-100 text-yellow-800"}`}
                        >
          ...
          <div className="text-center py-8 bg-gray-50 rounded-xl">
            ...
            <p className="text-sm text-gray-400">
              Use the form above to create your first article!
            </p>
            <div className="mt-4 flex justify-center space-x-2">
              <Sparkles className="w-6 h-6 text-yellow-500 animate-float" />
              <Heart
                className="w-6 h-6 text-red-500 animate-float"
                style={{ animationDelay: "0.2s" }}
              />
              <Sparkles
                className="w-6 h-6 text-blue-500 animate-float"
